class Simuploader extends SimpleModule
  ...
  upload: (file, opts = {}) ->
    ...
    startUploadFunc = ((_this)->()->
      return if _this.triggerHandler('beforeupload', file) == false
    ...
    if not @opts.resize?
      ...
      (lrz(file.obj, @opts.resize).then (rst)->
        file.obj = rst.file
        file.size = rst.fileLen).always ()->
          startUploadFunc()
  ...
  _xhrUpload: (file) ->
    ...
    file.xhr = $.ajax
      ...
      success: (result) =>
        if @opts.resultConvertFunc? && $.isFunction @opts.resultConvertFunc
          result = @opts.resultConvertFunc result
        if @opts.filePrefix?
          result.file_path = @opts.filePrefix + result.file_path
